 ...
 * Hash:   SHA-256.
 * Apple GrandSlam variant (validated byte-for-byte against the MIT `srp`
 * Python library, which authenticates live against Apple's GSA servers):
 *   - noUsernameInX: the username is dropped from x but the ":" separator is
 *     RETAINED, so x = H(s | H(":" | password_key)).
 *   - the "password_key" is the s2k/s2k_fo PBKDF2 output (see gsa_srp_s2k).
 *   - the (real) username IS folded into M1 via H(I).
 *   - A, B and S enter the hashes as MINIMAL big-endian bytes (no left-pad);
 *     only the k and u inputs are PAD-ed to len(N).
 *
 * SRP-6a math (Apple variant):
 *   A  = g^a mod N
 *   u  = H(PAD(A) | PAD(B))
 *   k  = H(PAD(N) | PAD(g))
 *   x  = H(s | H(":" | password_key))         [noUsernameInX, colon retained]
 *   S  = (B - k*g^x)^(a + u*x) mod N
 *   K  = H(S)                                 [S minimal big-endian]
 *   M1 = H( H(N) XOR H(g) | H(I) | s | A | B | K )   [A,B minimal big-endian]
 *   M2 = H( A | M1 | K )                      [A minimal big-endian]
 *
 * Pinned by tests/test_srp.c against tests/vectors/apple_srp_vector.h
 * (golden vectors generated by tests/oracle/gsa_oracle.py).
 */
#include "gsa/srp.h"
...
int gsa_srp_process(gsa_srp_ctx *ctx,
    ...
    /* x = H(salt | H(":" | password_key))
     * noUsernameInX drops the *username* from the inner hash but RETAINS the
     * ":" separator (verified against pysrp/Apple: inner = H("" + ":" + p)). */
    {
        uint8_t inner[GSA_SHA256_LEN];
        {
            ...
            sha256_begin(&ic);
            sha256_add(&ic, ":", 1);
            sha256_add(&ic, password_key, password_key_len);
            sha256_end(&ic, inner);
        }
    ...
    /* K = H(S)  -- minimal big-endian bytes of S (pysrp long_to_bytes; no PAD). */
    {
        size_t slen = bn_bytes(S);
        uint8_t *S_min = malloc(slen ? slen : 1);
        if (!S_min) goto done;
        BN_bn2bin(S, S_min);
        gsa_sha256(S_min, slen, ctx->K);
        free(S_min);
        ctx->have_K = 1;
    }
     ...
     * NOTE: H(I) inclusion under noUsernameInX is VALIDATION-PENDING. */
    {
        ...
        /* A and B enter M1 as minimal big-endian bytes (pysrp long_to_bytes). */
        size_t Alen = bn_bytes(ctx->A), Blen = bn_bytes(B);
        uint8_t *A_min = malloc(Alen ? Alen : 1);
        uint8_t *B_min = malloc(Blen ? Blen : 1);
        if (!A_min || !B_min) { free(A_min); free(B_min); goto done; }
        BN_bn2bin(ctx->A, A_min);
        BN_bn2bin(B, B_min);
        ...
        sha256_add(&c, A_min, Alen);
        sha256_add(&c, B_min, Blen);
        sha256_add(&c, ctx->K, sizeof(ctx->K));
        sha256_end(&c, ctx->M1);
        ctx->have_M1 = 1;
        memcpy(out_m1, ctx->M1, GSA_SHA256_LEN);
        free(A_min); free(B_min);
    }
...
int gsa_srp_verify(gsa_srp_ctx *ctx, const uint8_t M2[32]) {
    ...
    /* M2 = H( A | M1 | K )  -- A as minimal big-endian bytes (pysrp long_to_bytes). */
    size_t Alen = bn_bytes(ctx->A);
    uint8_t *A_min = malloc(Alen ? Alen : 1);
    if (!A_min) return -1;
    BN_bn2bin(ctx->A, A_min);
